    ...
    if($factura=='1'){
        ...
        try{
            ...
            if($resultado != false){
                ...
                require('../../basedatos/bdsqli.php');
                $stmt = $connf->prepare('UPDATE usuarios SET inscripcion_usuario = ?, deposito_usuario = ?, transferencia_usuario = ?, factura_usuario = ? WHERE correo_usuario = ?' );
                                $stmt->bind_param('iiiis', $inscripcion, $deposito,$trasnferencia,$factura, $mail);
                                $stmt->execute();         
                $stmt->close();
                $connf->close();
            }else{
